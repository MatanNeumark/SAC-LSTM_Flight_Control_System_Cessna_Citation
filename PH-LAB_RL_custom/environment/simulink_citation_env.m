classdef simulink_citation_env < handle
    properties
        mdl
    ...
    methods
        function obj = simulink_citation_env(ref_state, dt, steps_per_episode, state_vars, tracked_states, SA_noise_bias, SA_delay, incremental, servo_RL, servo_TF, mdl)
            env_config;
            initcit
            ...
            if all(~ismember([p, r, phi, Beta], state_vars)) && min(size(ref_state)) == 1 && incremental == true % if longitudinal
                ...
                ObservationInfo = rlNumericSpec([1 length(state_vars) + length(tracked_states) + 2 * ActionInfo.Dimension(2)]);
            ...
            obj.servo_RL = servo_RL;
            obj.servo_TF = servo_TF;
            ...
            obj.bounds.ref_states_idx = length(state_vars) + 1:length(state_vars) + obj.bounds.n_ref_states;
            ...
            obj.outvec = obj.state_vars_dict(obj.state_vars);
            obj.x0 = x0;
        ...
        function [new_state, reward, terminated, Info, applied_action, internal_state] = step(obj, action)
            ...
            obj.sm = setBlockParameter(obj.sm, obj.elevator_input, "value" ,num2str(applied_action));
            % advance the environment by one time step step_size
            step(obj.sm, PauseTime= obj.sm.Time + obj.dt);
            s = obj.sm.SimulationOutput.xout.signals(1).values(end,:);
            s = s(obj.state_vars) - obj.x0(obj.state_vars);
            internal_state = s;
            s = obj.sensor_dynamics(s);

            if obj.incremental
                new_state = [s, obj.ref_state(obj.counter+1,:), applied_action', obj.last_action];
            else
            ...
            reward = obj.reward_function(s, action, terminated);
        ...
        function [state_0, action_0] = reset(obj)
            ...
            obj.sm = setBlockParameter(obj.sm, obj.elevator_input, "value" ,mat2str(obj.x_servos));
        ...
        function reward = reward_function(obj, new_state, action, terminated)
            if ~terminated %|| (terminated && ismember(obj.tracked_states, obj.state_vars(obj.lim_exceeded)))
                ...
                %reward = -14*sum((obj.ref_state(obj.counter+1,:) - new_state(obj.tracked_idx)).^2);
