classdef Linear_Citation_env_for_LSTM_xdot < rl.env.MATLABEnvironment
    properties
        state_vars_dict = dictionary(1, "p", 2, "q", 3, "r", 4, "V_TAS", 5, "alpha", 6, "beta", 7, "phi", 8, "theta", 9,  "psi", 10, "he", 11, "xe", 12, "ye",...
                                     13, "p_dot", 14, "q_dot", 15, "r_dot", 16, "V_TAS_dot", 17, "alpha_dot", 18, "beta_dot", 19, "phi_dot", 20, "theta_dot", 21,  "psi_dot", 22, "he_dot", 23, "xe_dot", 24, "ye_dot")
    ...
    methods              
        function obj = Linear_Citation_env_for_LSTM_xdot(ref_state, dt, steps_per_episode, state_vars, tracked_states, SA_noise_bias, SA_delay, incremental, training)
            env_config;
            ...
            elseif all(ismember([p, q, r, phi, Beta], state_vars)) && min(size(ref_state)) == 3 && incremental == true % if longitudinal and lateral
                ...
                ActionInfo = rlNumericSpec([1 3], LowerLimit=[de_lo_sat_lim, -da_sat_lim, -dr_sat_lim], UpperLimit=[de_up_sat_lim, da_sat_lim, dr_sat_lim]);
                ObservationInfo = rlNumericSpec([1 length(state_vars) + length(tracked_states) + ActionInfo.Dimension(2)]);
            ...
            obj.x_vars = state_vars(state_vars<=12);
            obj.xdot_vars = state_vars(state_vars>12);
            obj.state_vars = state_vars;
            obj.tracked_states = tracked_states;
            obj.SA_noise_bias = SA_noise_bias;
            obj.SA_delay = SA_delay;
            ...
            obj.bounds.n_ref_states = length(tracked_states);
            obj.bounds.ref_states_idx = length(obj.state_vars) + 1:length(obj.state_vars) + obj.bounds.n_ref_states;
            %load Matan_v90ms_m4500kg_h2000m_q_init.lin -mat
            %load('environment/full_DoF_v90ms_m4500kg_h2000m_q_init.lin', '-mat')
            load('environment/full_DoF_x_xdot_v90ms_m4500kg_h2000m_q_init.lin', '-mat')
            ...
            obj.servos.rate_lim = [de_rate_lim, da_rate_lim, dr_rate_lim];
            obj.servos.rate_lim = obj.servos.rate_lim(1:obj.bounds.action_space_size);
            obj.servos.tau = 1/13;
            obj.u0 = u0;
            ...
            A = zeros(length(obj.state_vars));
            A(1:length(obj.x_vars), 1:length(obj.x_vars)) = Alin(obj.x_vars, obj.x_vars);
            B = Blin(obj.x_vars, 1:ActionInfo.Dimension(2));
            B = [B; 0];
            Clin = [Clin, zeros(height(Clin), width(Clin))];
            C = Clin(obj.state_vars, obj.state_vars);
            D = Dlin(obj.state_vars, 1:ActionInfo.Dimension(2));
            ...
            obj.bounds.states_upper_bounds = [deg2rad(20), deg2rad(10), deg2rad(10), 130, deg2rad(10), deg2rad(10), deg2rad(20), deg2rad(10), inf, 2300, inf, inf,...
                                              deg2rad(40), deg2rad(40), deg2rad(40), 130, deg2rad(40), deg2rad(40), deg2rad(40), deg2rad(40), inf, 2300, inf, inf];
            obj.bounds.states_lower_bounds = [-deg2rad(20), -deg2rad(10), -deg2rad(10), 50, -deg2rad(10), -deg2rad(10), -deg2rad(20), -deg2rad(10), -inf, 1700, -inf, -inf,...
                                              -deg2rad(40), -deg2rad(40), -deg2rad(40), 50, -deg2rad(40), -deg2rad(40), -deg2rad(40), -deg2rad(40), -inf, 1700, -inf, -inf];
        ...
        function [new_state, reward, terminated, Info, applied_action, internal_state] = step(obj, action)
            ...
            applied_action = obj.servo_dynamics(action)';
            s = ((obj.lin_sys.A*obj.dt + obj.I)*obj.state' + obj.lin_sys.B*obj.dt*applied_action)';
            obj.state = s;
            y = (obj.lin_sys.C*s' + obj.lin_sys.D*applied_action)';
            y = obj.sensor_dynamics(y);
            %s = obj.sensor_dynamics(s);

            % 
            if obj.incremental
                %new_state = [s, obj.ref_state(obj.counter+1,:), applied_action'];
                new_state = [y, obj.ref_state(obj.counter+1,:), obj.last_action];
            else
                new_state = [y, obj.ref_state(obj.counter+1,:)];
            end

            internal_state = obj.state;    
            terminated = obj.termination_function(s);
            reward = obj.reward_function(y, action, terminated);

            obj.is_terminated = terminated;
            obj.counter =  obj.counter + 1;
            %obj.last_state = s;
            %obj.last_desired_action = action;
        end
        ...
        function [state_0, action_0] = reset(obj)
            ...
            if obj.training == true
                ...
                max_amp = 5; % degrees
                max_freq = 0.2;%0.4;
                obj.ref_state(:,idx) = deg2rad(max_amp-2*max_amp*rand) * sin(max_freq*rand*pi*obj.ep_length');
                obj.ref_state = obj.ref_state(:,obj.tracked_idx);
                for i = 1:length(obj.x_vars)
       ...
       function stop(obj)
        ...
        function reward = reward_function(obj, new_state, action, terminated)
            ...
            if ~terminated %|| (terminated && ismember(obj.tracked_states, obj.state_vars(obj.lim_exceeded)))
                %reward = -(180/pi)*sum((obj.ref_state(obj.counter+1,:) - new_state(obj.tracked_idx)).^2);% - 6*sum(action.^2);
                reward = -14*sum((obj.ref_state(obj.counter+1,:) - new_state(obj.tracked_idx)).^2) - 10*q_dot^2;
                reward = clip(reward, -1, 0);
            else
                reward = -1; % max error can be 25 deg, resulting in a reward of 10.9. So penalty should be larger.
            end            
        end
        ...
        function terminated = termination_function(obj, new_state)
            ...
            if any(lim_exceeded)
              ...
              disp("Terminating because a limit is exceeded for state: " + var_id + new_state(lim_exceeded)')
            else
              terminated = false;
            end
        end
        ...
        function action = servo_dynamics(obj, action)
            if obj.incremental
                action = obj.last_action + action - obj.servo_bias;
            ...
            % action = clip(action, obj.last_action-obj.servos.rate_lim*obj.dt, obj.last_action+obj.servos.rate_lim*obj.dt); % delete if using incremental;
            ...
            % if obj.S_A_dynamics % alternative model
        ...
        function noisy_state = sensor_dynamics(obj, new_state)
            ...
            if any(obj.b_rates_logical)
                ...
                noisy_state(nonzeros(obj.b_rates_idx)) = b_rate_mean + obj.b_rate_bias + obj.b_rate_std*randn([1, length(nonzeros(obj.b_rates_idx))]);
                obj.b_rate_buffer(1,:) = [];
            end
            if any(obj.V_TAS_logical)
                obj.V_TAS_buffer(end+1,:) = new_state(nonzeros(obj.V_TAS_idx));
                V_TAS_mean = obj.V_TAS_buffer(1,:); % with delay
                noisy_state(nonzeros(obj.V_TAS_idx)) = V_TAS_mean + obj.V_TAS_bias + obj.V_TAS_std*randn([1, length(nonzeros(obj.V_TAS_idx))]);
                obj.V_TAS_buffer(1,:) = [];
            end
            if any(obj.euler_logical)
                obj.euler_buffer(end+1,:) =  new_state(nonzeros(obj.euler_idx));
                euler_mean = obj.euler_buffer(1,:); % with delay
                noisy_state(nonzeros(obj.euler_idx)) = euler_mean + obj.euler_bias + obj.euler_std*randn([1, length(nonzeros(obj.euler_idx))]);
                obj.euler_buffer(1,:) = [];
            end
            if any(obj.aero_logical)
                obj.aero_buffer(end+1,:) = new_state(nonzeros(obj.aero_idx));
                aero_mean = obj.aero_buffer(1,:); % with delay
                noisy_state(nonzeros(obj.aero_idx)) = aero_mean + obj.aero_bias + obj.aero_std*randn([1, length(nonzeros(obj.aero_idx))]);
                obj.aero_buffer(1,:) = [];
            end
            if any(obj.alt_logical)
                obj.alt_buffer(end+1,:) = new_state(nonzeros(obj.alt_idx));
                alt_mean = obj.alt_buffer(1,:); % with delay
                noisy_state(nonzeros(obj.alt_idx)) = alt_mean + obj.alt_bias + obj.alt_std*randn([1, length(nonzeros(obj.alt_idx))]);
                obj.alt_buffer(1,:) = [];
            end
        end
    end
